...
SELECT 
 ...
 SELECT 
    ...
    ,2) AS Avg_discharge_Length_Stay FROM V_admissiondata;
-- 7
...
-- Creating View - of patients who got readmitted within 30 days 

CREATE OR REPLACE VIEW V_Readmission_30 AS
WITH CTE AS (
SELECT *, LAG(clean_D_O_D) OVER (PARTITION BY MRD_NO ORDER BY clean_D_O_A) AS prev_discharge 
FROM V_admissiondata)
SELECT *, CASE 
               WHEN DATEDIFF(clean_D_O_A , prev_discharge ) BETWEEN 1 AND 30 THEN 1 ELSE 0 END AS Readmission_30 FROM 
               CTE ;
               
-- Q9 READMISSION COUNT WITHIN 30 DAYS OF DISCHARGE 

SELECT SUM(Readmission_30)FROM V_Readmission_30 WHERE Prev_discharge  is not null;    
-- 616

-- Q9 READMISSION RATE WITHIN 30 DAYS OF DISCHARGE           
               
SELECT ROUND(SUM(Readmission_30)*100/ COUNT(*),2) AS Readmissionrate_30 FROM V_Readmission_30;
-- 4.62%

-- Q10 COUNT OF PATIENTS WITH ONLY K/C/O HTN GOT READMITTED  WITHIN 30 DAYS OF DISCHARGE 

SELECT SUM(Readmission_30) AS Readmission30_pt_with_HTN FROM V_Readmission_30 WHERE HTN = 1 AND DM = 0;
-- 167


-- Q11 COUNT OF PATIENTS WITH ONLY K/C/O DM GOT READMITTED  WITHIN 30 DAYS OF DISCHARGE
 
SELECT SUM(Readmission_30) AS Readmission30_pt_with_DM FROM V_Readmission_30 WHERE DM = 1 AND HTN = 0;

-- 74

-- Q12 COUNT OF PATIENTS WITH K/C/O DM AND HTN GOT READMITTED WITHIN 30 DAYS OF DISCHARGE 

SELECT SUM(Readmission_30) AS Readmission30_with_DM_HTN FROM V_Readmission_30 WHERE DM AND HTN = 1;
